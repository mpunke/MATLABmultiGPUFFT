%% Single FFT with multiple GPU usage for PFC (2D)
clearvars
...
%% intial density
% the density field is initialized as a circular seed
...
psiIF = psi0M;
psiF = fft2(psiIF);
lin =  (lambda-kappa+kappa*(1+lap).^2).*lap;
...
spmd
    ...
    for s =1:tStepEuler %time iteration
        %%forward fft2
        psi = fft(-1/2.*delta.*psi.^2+1/3.*psi.^3);%fft along first dimension
        ...
        for iter = 1:NGPU%decompose each slab in a set of local chunks and
            ...
            if iter==1
                ...
                psiTest1= spmdCat(psi1,2,1);
            elseif iter==2
                psi2 = psi(start2:end2,:);
                psiTest2= spmdCat(psi2,2,2);
            elseif iter==3
                psi3 = psi(start3:end3,:);
                psiTest3= spmdCat(psi3,2,3);
            elseif iter==4
                psi4 = psi(start4:end4,:);
                psiTest4= spmdCat(psi4,2,4);
            elseif iter==5
                psi5 = psi(start5:end5,:);
                psiTest5= spmdCat(psi5,2,5);
            elseif iter==6
                psi6 = psi(start6:end6,:);
                psiTest6= spmdCat(psi6,2,6);
            elseif iter==7
                psi7 = psi(start7:end7,:);
                psiTest7= spmdCat(psi7,2,7);
            elseif iter==8
                psi8 = psi(start8:end8,:);
                psiTest8= spmdCat(psi8,2,8);
            end
        end
        ...
        psi = fft(psi,[],2);%fft along third dimension

        %%semi-implicit time step update
        psiF = (psiF+deltatEuler.*lap.*psi)./(1-deltatEuler.*lin);

        psi = psiF;

        %%backward ifftn
        psi  = ifft(psi,[],2);%ifft along third dimension
        ...
        psi  = ifft(psi);%ifft along first dimension
    end

end

clear lap lin psiF psiTest1 psiTest2 psiTest3 psiTest4 psi1 psi2 psi3 psi4
%% stack solution
for k = 1:numel(psi)
    psiGPU{k} = gather(psi{k});
end
psiSol = cat(2, psiGPU{:});
save('GPUSolution.mat','psiSol')
